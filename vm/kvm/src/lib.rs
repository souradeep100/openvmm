...
impl Partition {
    /// Borrow the KVM VM fd without transferring ownership to the caller.
    ///
    /// This is analogous to handing out a borrowed `int fd` in C, except Rust
    /// ties the returned fd lifetime to `&self`.
